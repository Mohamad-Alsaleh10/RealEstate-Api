            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.users.update', $user) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="password" class="block text-sm font-medium text-gray-700">{{ __('كلمة المرور (اتركها فارغة لعدم التغيير)') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="password_confirmation" class="block text-sm font-medium text-gray-700">{{ __('تأكيد كلمة المرور') }}</label>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="role" id="role" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                <option value="user" @selected(old('role', $user->role) == 'user')>{{ __('مستخدم عادي') }}</option>
                                <option value="admin" @selected(old('role', $user->role) == 'admin')>{{ __('مسؤول') }}</option>
                            </select>
                            @error('role')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
